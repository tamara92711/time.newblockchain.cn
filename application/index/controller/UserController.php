...
class UserController extends Controller
{
    ...
    public function submit_login(Request $request)
    {
        ...
        if (UserModel::where('mobile',$mobile)->count() == 0)
        {
            ...
            if ($user_record->password != md5($password))
            {
                ...
                UserModel::where('id',session('user_id'))->update(['last_login'=>date('Y-m-d H:i:s'),'last_ip'=>$request->ip()]);
    ...
    public function create_phone_verify_code1(Request $request)
    {
        $send_status = [
            "0" => "短信发送成功",
            "-1" => "参数不全",
